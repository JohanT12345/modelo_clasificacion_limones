...
EPOCHS   = 100
IMG_SIZE = 640
MODELO   = "yolov8n-cls.pt"   # nano = más rápido; cambia a yolov8s-cls.pt si quieres más precisión
...
def construir_dataset_yolo(carpeta_aumentada: Path, carpeta_dataset: Path):
    """
    Organiza las imágenes en la estructura que espera YOLOv8 classify:
        limon_dataset/
            train/
                exportacion/
                consumo_interno/
...
def entrenar(ruta_yaml: Path):
    ...
    resultados = modelo.train(
        data    = str(ruta_yaml.parent),   # YOLOv8 classify usa la carpeta raíz
        epochs  = EPOCHS,
        imgsz   = IMG_SIZE,
        batch   = 16,
        patience= 20,         # Early stopping si no mejora en 20 epochs
        workers = 0,          # En Windows usar 0 para evitar errores multiprocessing
        project = "runs/classify",
        name    = "limon_madurez",
        exist_ok= True,
        plots   = True,       # Genera gráficas de entrenamiento automáticamente
        verbose = True,
        device=0,
    )
    return resultados
